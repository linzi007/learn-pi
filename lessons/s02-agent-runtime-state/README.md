# s02：运行状态（Agent Runtime State）— UI 不必自己拼运行状态

[返回首页](../../README.md)

[s01 Pi 运行管理器](../s01-pi-agent-turn/) → `s02` → [s03 工具执行管线](../s03-tool-execution-pipeline/) → 后续课程

> *“收到事件时，Pi 的运行状态（`AgentState`）已经更新。”*
>
> **pi-agent-core 层**：模型事件流解决“模型刚输出了什么”；运行器（`Agent`）再解决“这次运行走到哪里、界面现在该显示什么”。

推荐前置：已完成 `learn-claude-code/s01_agent_loop`，知道一次提问（`prompt`）会驱动模型与后续工作。本课不重写 Agent Loop，而是只看 Pi 为界面（UI）、SDK 和宿主程序提供的生命周期与状态契约。

---

## 这节只学什么

本课只解决“界面收到一次事件后，怎样直接读取 Pi 已经整理好的运行状态”这个问题。

| 本课会看到 | 本课暂不展开 |
| --- | --- |
| 正在生成的文字、已完成历史、运行中与失败状态 | 模型怎样选择工具，留给 s03 |
| 事件发生后直接读取 `agent.state` | 会话怎样持久化，留给 s08 |
...
```text
刚发送        -> 显示用户消息，按钮进入运行中
模型持续输出  -> 更新一条尚未完成的助手回复气泡
整轮结束      -> 固化完整回复，按钮恢复可用
```

如果界面（UI）自己根据底层文字增量（`text_delta`）拼出这些状态，很容易犯错：把每个增量当成一条新消息、在完整回复前写入历史，或在模型已经结束后仍让发送按钮处于运行中（loading）。

Pi 如何把底层模型事件变成让界面直接消费的运行进度、消息变化和状态快照？
...
*图：模型事件进入 Agent 的状态归约逻辑后，订阅者读到的 `AgentState` 已经可以直接驱动运行中、未完成消息和历史记录。*

运行器（`Agent`）的关键约定是：**先更新当前状态（`agent.state`），再通知订阅者（`subscribe()` listener）。** 因此订阅者收到的不是“请你自己更新状态”的命令，而是“状态已经变化”的通知。

| 你想驱动的界面 | 读取什么 |
| --- | --- |
| 发送按钮是否运行中 | `agent.state.isStreaming` |
| 正在生成的气泡 | `agent.state.streamingMessage` |
| 已完成的聊天历史 | `agent.state.messages` |
| 本次运行是否失败 | `agent.state.errorMessage` 的存在性 |

一句话结论：**事件描述发生了什么；状态描述界面现在应该画什么。**

---

## 工作原理

完整教学代码在 [`code.ts`](code.ts)。文件开头创建真实模型，只回答“这次请求发给谁”；本课真正要读的是 `observeAgentRun()`：它不维护第二份界面状态，只在每次事件到达时读取 Pi 已经更新好的 `agent.state`。

### 第 1 步：订阅时，只读取已经更新的状态
...
这段代码没有一份手写的“运行中”、未完成文本或历史数组。每收到一个运行事件（`AgentEvent`），它只做一件事：记录**此刻**的 `agent.state`。因此终端、文本界面（TUI）和嵌入式宿主可以各自渲染同一个状态，而不必分别猜测某个事件会怎样改写历史。

### 第 2 步：一次提问（`prompt()`）先把运行状态打开

```ts
await agent.prompt(prompt);
```

调用开始后，第一条关键观察不是模型文字，而是 `agent_start` 的状态快照：`isStreaming` 已经为 `true`。随后用户消息先经过临时区，再进入已完成历史：

![一轮 Agent 生命周期中的事件和状态变化](images/lifecycle-timeline.svg)

*图：事件和状态成对变化；`agent_end` 表示循环停止，真正空闲要等内部 `finishRun()` 清理运行状态。*

按 `code.ts` 记录的真实顺序，界面能看到这些变化：

1. `agent_start`：`isStreaming` 已经是 `true`，界面可进入运行中。
2. `message_start(user)`：未完成消息（`streamingMessage`）暂时指向 user；还没有写进历史记录（`messages`）。
3. `message_end(user)`：user 被追加到 `messages`，临时消息清空。
4. `message_start(assistant)`：临时区换成一条尚未完成的助手消息。

### 第 3 步：文字增量只改临时消息，完整消息才进入历史
...
因此 `message_update(text_delta)` 再多，`transcript` 仍只有 user；它们只持续替换 `streamingMessage`。直到 `message_end(assistant)`，完整助手消息才被追加到 `messages`。这正是页面同时显示“正在回答”和“已完成历史”时不需要自己拼文字增量的原因。

底层的助手消息事件（`AssistantMessageEvent`）仍挂在 `message_update` 上。需要画 token/文字增量的界面可以读取它；只需要画气泡的界面则只需读这两个状态区域。

### 第 4 步：`agent_end` 不是按钮恢复可用的边界

`code.ts` 在 `agent_end` 订阅者中记录 `isStreaming`。你会看到它仍然是 `true`：

```text
agent_end(本次新增 2 条消息) | isStreaming=true, streamingMessage=-, transcript=user -> assistant
```

这不是延迟更新。`agent_end` 的含义是“循环不再发事件”；Pi 还会等待所有异步订阅者完成，随后由内部收束逻辑清理运行状态。

所以 `observeAgentRun()` 先 `await agent.prompt(prompt)`，再读取最终状态快照。让按钮恢复可用的可靠边界是这个 Promise 返回后，或 `await agent.waitForIdle()` 完成后；不要只看见 `agent_end` 就判定空闲。

### 第 5 步：失败也要收束为可画的状态

当模型服务（Provider）没有完成响应时，运行器仍会保留一条助手消息，并走到 `turn_end`、`agent_end`。状态快照只暴露 `hasError=true`，课程输出不会把 Provider 原始细节写到终端或日志。

这让界面能同时做到两件事：保留用户刚问过的问题和失败位置；用通用的“本次模型调用未完成”提示替代一个停不下来的运行中状态。

> **可复述的规则**：Pi 的状态归约逻辑（Agent reducer）负责先决定“现在是什么状态”，订阅者负责读取它；只有运行收束后，`isStreaming` 才会回到 `false`。

---

## 试一下

本课默认需要 Node.js `>=22.19.0` 和有效的 Anthropic 兼容配置。可用环境变量与 s01/s03 一致：
...
[步骤 2/4] Pi 开始本轮：事件到达前，运行状态已经切换为运行中。
...
[步骤 3/4] 首个文字增量到达：只更新临时助手消息，不写入完整历史。
...
真实调用可能产生模型费用。无有效配置或模型请求失败时，入口只输出通用中文提示并以非零状态结束，不打印模型服务的原始请求或认证细节。
...
1. 多个底层文字增量被归约为一条完整助手消息和稳定历史记录。
2. 离线伪模型（faux Provider）错误仍走完 `agent_end`，观察输出不泄露原始错误内容。

可以尝试：

1. 设置 `LEARN_PI_PROMPT` 为一个更长的问题，观察 `message_update` 的数量。
2. 给订阅者（`subscribe()` listener）加一个短暂异步等待，观察 `agent_end` 与真正空闲的边界。
3. 在开发者工具中分别用 `streamingMessage` 和 `messages` 渲染两块区域，比较未完成与已完成消息。
...
[s03 工具执行管线](../s03-tool-execution-pipeline/) 将沿着这条消息生命周期之后的支路继续，解释为什么工具完成顺序和历史写入顺序可以不同。
...
| --- | --- |
| 创建 `Agent` 与 `await agent.prompt(prompt)` | [`Agent.prompt()`](https://github.com/earendil-works/pi/blob/2b3fda9921b5590f285165287bd442a25817f17b/packages/agent/src/agent.ts#L269-L281) 启动一次运行；[`runWithLifecycle()`](https://github.com/earendil-works/pi/blob/2b3fda9921b5590f285165287bd442a25817f17b/packages/agent/src/agent.ts#L469-L525) 在结束时协调状态清理。 |
| 在 `agent.subscribe` 中读取 `agent.state` | [`processEvents()`](https://github.com/earendil-works/pi/blob/2b3fda9921b5590f285165287bd442a25817f17b/packages/agent/src/agent.ts#L527-L580) 先把事件归约进状态，才 `await` 订阅者；这就是本课不手写界面状态的依据。 |
