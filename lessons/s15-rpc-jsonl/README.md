# s15：RPC 的逐行 JSON 通道（JSONL）— 响应和事件怎样不串线

[← s14 终端差分渲染](../s14-tui-diff-render/README.md) · [返回首页](../../README.md)

> **核心结论**：RPC 客户端（`RpcClient`）给每条命令一个编号；带相同编号的响应只交还给原来的请求，Agent 的过程事件则单独通知界面。

推荐前置：已完成 s13，知道 RPC 是 Pi 的一种运行入口。本课不重新实现通用 Agent Loop；只观察宿主程序怎样可靠地控制独立的 Pi 子进程。

---

## 这节只学什么

本课只解决“一条标准输出同时出现响应和过程事件时，宿主怎样不把它们串在一起”这个问题。

| 本课会看到 | 本课暂不展开 |
| --- | --- |
| 请求编号把并发响应交还给正确的调用方 | JSON 序列化、字符边界和完整协议兼容细节 |
| 接受问题与 Agent 完成是两个不同信号 | RPC 支持的所有会话、工具和界面命令 |

逐行 JSON 中 Unicode 分隔符的边界规则由离线测试覆盖，并放在深入源码区说明；主线先建立请求编号和事件分流这条规则。
...
两条命令都写入子进程的 stdin，两个答案却从同一条 stdout 回来。它们不保证按发送顺序出现：先写出的 `get_state`，不代表一定先读到它的响应。

随后 IDE 再发送问题。此时 stdout 又会连续出现 `agent_start`、`message_update`、`agent_settled` 等过程事件。问题的接受响应只说明 Pi 已收到输入，并不表示模型已经完成。

所以宿主不能把“stdout 的下一行”当作“刚才那条命令的答案”。它必须同时解决两个问题：

1. 两个并发查询怎样各自回到发起它的等待结果
2. 一条问题怎样在很早收到“已接受”后，仍持续收到 Agent 过程事件
...
*图：`req_2` 先返回只是协议允许的一种交错顺序，实际先后取决于命令耗时。无论顺序怎样，响应按请求编号回到对应等待结果；Agent 事件继续流向监听器。*

先用一句白话定义 JSONL：**一行就是一条 JSON 记录**。Pi 的 stdout 会混合两种记录；客户端不能只按“第几行”理解它：

| stdout 中读到的记录 | 客户端怎样判断 | 交给谁 |
| --- | --- | --- |
| 响应记录（`response`），且编号仍在等待表中 | 这是某条尚未完成命令的答案 | 该编号对应的等待结果（Promise） |
| `agent_start`、`message_update`、`agent_settled` 等 Agent 记录 | 它不是等待中的响应 | 事件监听器，例如 `collectEvents()` |
...
前两行展示“答案可以乱序”；后四行展示“接受问题”和“Agent 完成”是两个不同信号。本课先等待两条查询完成，再发送问题，因此图中的 Agent 事件不会被拿来伪装成两条查询之间的固定输出顺序。

一句规则：**请求编号决定哪一个等待结果拿到响应；事件类型描述 Agent 仍在做什么。它们共用 stdout，却不能共用同一种等待方式。**

---

## 工作原理

课程入口是 [`code.ts`](code.ts)。模型服务、临时目录和子进程参数只是让这段代码能够独立、真实地运行；它们不会改变下面这条因果链。先从读者实际要观察的结果开始。
...
`start()` 已经让 Pi RPC 子进程开始读取标准输入、写入标准输出。`Promise.all()` 的两个调用会立刻各自发出命令；代码不会先等待 `client.getState()`，再发第二条。
...
这里保持不变的是同一个 Pi 会话运行时（`AgentSessionRuntime`）；本课新增的是跨进程的请求关联。两个查询都只是 RPC 命令，真正保证不串线的决策发生在 `RpcClient` 内部。

### 第 2 步：先登记等待结果（Promise），再按编号交还响应

对每次 `getState()`、`getAvailableModels()` 这样的公开调用，Pi 的 `RpcClient.send()` 会依次做三件事。这里的等待结果（Promise）就是“这条命令尚未收到答案时，调用方暂时拿到的占位结果”：
...
收到标准输出的一条记录后，它不关心这是第几行，而是检查它是不是带有仍在等待编号的响应：
...
所以 `req_2` 即使先回来，也只会完成 `getAvailableModels()`；`getState()` 仍等自己的 `req_1`。这正是 `Promise.all()` 可以安全并发的原因。本课不人为延迟 Pi 命令来制造固定乱序，图里的顺序只说明协议允许什么。

### 第 3 步：先安装事件收集器，再发送问题
...
| `agent_settled` | 运行时已重新空闲 | `promptAndWait()` 返回收集到的事件 |
...
事件列表适合宿主边收到边更新界面；`getLastAssistantText()` 则再发送一条请求/响应，读取已经写入会话的最终 assistant 文本。两者仍走同一条 RPC 通道，但一个是持续订阅，一个是有结果的查询。
...
Pi 会把 `success: false` 和原请求编号一起写回。`RpcClient` 先找到正确的等待结果，再将它变成 JavaScript `Error`；错误不会混进 Agent 事件流。
...
再压缩一次本课规则：**`RpcClient` 负责把编号映射回等待中的命令；`AgentSession` 负责持续产生状态事件；宿主必须把“命令接受”与“Agent settled”当成两个不同节点。**
...
```text
[步骤 1/5] 启动隔离的 RPC 子进程，并建立标准输入/输出通道。
RPC 子进程: 已启动（model=learn-pi-anthropic-compatible/<MODEL_ID>）
[步骤 2/5] 并发发送两条查询：响应必须按请求编号归还，而非按到达顺序。
并发响应: get_state.sessionId=<动态 Session ID>, models=<大于 0>
[步骤 3/5] 发送问题：接受响应与 Agent 完成事件分开观察。
异步事件: agent_start -> ... -> message_update -> ... -> agent_settled
最终文本: <真实模型生成的中文结果>
[步骤 4/5] 故意请求不存在的模型：观察关联的错误响应。
错误响应: Model not found: missing-provider/missing-model
[步骤 5/5] 清理：停止 RPC 子进程并删除本次临时配置。
...
s13 选择不同的运行入口，s14 展示交互入口怎样更新终端，本课则让外部程序跨进程控制 Pi。到这里，运行入口与界面主线已经闭合：
